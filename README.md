...
2. Для работы со своими **валютами и акциями** запишите их в соответствующие списки в файле `user_settings.json` в корневой директории проекта.


3. Запустите программу в модуле `main.py` в корневой директории `src/`.
...
1. Функция `spending_by_workday` считает средние траты в рабочий и в выходной день за последние три месяца (от переданной даты).
